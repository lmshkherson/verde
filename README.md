...
app/
  (shop)/            вітрина: спільний макет із шапкою, підвалом і кошиком
    ...
    catalog/         каталог дизайнів із фасетними фільтрами
    roboty/          готові роботи: конкретне авто з реальними фото
  ...
  admin/
    ...
    (panel)/         панель: огляд, замовлення, заявки, відгуки,
                     готові роботи, лінійки, база авто, блог
...
**Дизайн (`/product/{лінійка}`).** Ілюструємо крій і даємо зібрати свій варіант:
матеріал, основний колір, колір вставок, колір строчки, вишивка логотипа. Схема
сидіння перемальовується під вибір, ціна перераховується під кузов авто.

**Готова робота (`/roboty/{slug}`).** Конкретний автомобіль, реальні фото після
встановлення, фіксована ціна. Заповнюється в адмінці: `/admin/works` — вибір
марки й моделі, завантаження фото авто та фото чохлів у салоні, ціна й опис.
Робота автоматично зʼявляється на сторінці своєї моделі авто, на головній
(якщо позначена «на головній») і в `sitemap.xml`.

Фото зберігаються в `public/uploads` і віддаються статикою. Для serverless-
хостингу без постійного диска треба замінити `saveUpload` у `lib/uploads.ts`
на завантаження в S3 — решта коду про сховище не знає.

## Ключові рішення

**Ціноутворення — в одному місці.** Ціна конфігуратора фіксована за варіантом
комплекту (`SeatSet` × `Series` → `SeriesPrice`) і не залежить від марки авто:
марка визначає лише лекала. Зверху додаються доплата за матеріал (різниця з
базовим матеріалом лінійки), за рідкісні кольори та обрані опції. Варіанти:
передні 1+1, передні 1+2 (мікроавтобуси), повний на 5 і на 7 місць — доступні
фільтруються під кількість місць у салоні. Готові роботи мають власну ціну
на кожній картці й у цю схему не входять.

**Ціни перевіряються на сервері.** `createOrder` бере ціну варіанта комплекту
з бази, перераховує опції за прайсом і перевіряє, що доплати вкладаються в
можливий діапазон. Для готових робіт ціна звіряється з карткою за її slug.
Підмінити суму з консолі браузера не вийде.
...
## SEO та видимість в AI-пошуку

- 287 сторінок моделей генеруються з даних і потрапляють у `sitemap.xml`.
- Мікророзмітка: AutoPartsStore (LocalBusiness) + WebSite глобально; Product з
  AggregateOffer і FAQPage на сторінках моделей; Product на картках і готових
  роботах; Article у блозі; BreadcrumbList на глибоких сторінках.
- FAQ-блоки генеруються з бази (ціни, строки, комплектація), тому не
  розходяться з прайсом. Загальна сторінка `/faq` — 13 питань покупця.
- `robots.txt` явно дозволяє AI-краулери (GPTBot, ClaudeBot, PerplexityBot,
  Google-Extended та ін.) — вони джерело відповідей асистентів.
- `/llms.txt` — конспект сайту для LLM за форматом llmstxt.org: факти, ціни,
  структура каталогу. Генерується з бази, кешується на годину.
